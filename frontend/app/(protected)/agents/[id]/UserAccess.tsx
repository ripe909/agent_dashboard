    ...
    <div>
      <div className="flex items-center justify-between">
        {enabled ? (
          ...
          <span className="text-sm text-[var(--text-secondary)] italic">Users can't sign in through this agent yet</span>
        )}
        {!enabled && (
          <button
            onClick={enable}
      ...
      {error && <div className="text-xs text-red-600 bg-red-50 border border-red-200 rounded px-3 py-2 mt-2">{error}</div>}
    </div>
  );
}
      ...
      <div className="mb-4">
        ...
        {loadingAssigned ? (
          ...
          <div className="space-y-2">
            {assignedUsers.map((u) => (
              <div key={u.id} className="flex items-center gap-3 bg-[var(--bg-surface-muted)] border border-[var(--border-default)] rounded-lg px-3 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-medium text-[var(--text-primary)] truncate">{u.displayName}</div>
      ...
      {open && (
        <div className="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-xl p-3 shadow-sm">
          ...
          <div className="max-h-56 overflow-y-auto space-y-0.5">
            {searching && <div className="text-xs text-[var(--text-secondary)] text-center py-4">Searching…</div>}
            ...
            {searchResults.map((u) => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-[var(--bg-surface-muted)] rounded-lg transition-colors text-left disabled:opacity-50 disabled:cursor-default"
                >
                  <div className="w-7 h-7 rounded-full bg-[#1662dd]/15 flex items-center justify-center text-xs font-bold text-[#1662dd] flex-shrink-0">
                    {initials(u.displayName)}
                  </div>
                  ...
                  {assigning === u.id ? (
                    <RefreshCw className="w-3.5 h-3.5 text-[#1662dd] animate-spin flex-shrink-0" />
                  ) : alreadyAssigned ? (
                    <span className="text-xs text-[var(--text-muted)] flex-shrink-0">Assigned</span>
                  ) : null}
                </button>
              );
            })}
